...
public class RegisterActivity extends Activity {
	...
	OnClickListener btRegister_OnClick = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			{
				Toast.makeText(getApplicationContext(), "用户帐号不可以为空!", 2000).show();
				return;
			}
			if(etUserName.getText().toString().trim().length() == 0)
			{
				Toast.makeText(getApplicationContext(), "用户姓名不可以为空!", 2000).show();
				return;
			}
			if(etUserPwd1.getText().toString().trim().length() == 0)
			{
				Toast.makeText(getApplicationContext(), "用户密码不可以为空!", 2000).show();
				return;
			}
			...
			{
				if(!etUserPwd2.getText().toString().equals(etUserPwd1.getText().toString()))
				{
			...
			{
				Toast.makeText(getApplicationContext(), "用户电话不可以为空!", 2000).show();
				return;
			}
			else
			{
				...
				{
					Toast.makeText(getApplicationContext(), "用户电话号码格式不正确!", 2000).show();
					return;
				}
				if(etPhone.getText().toString().trim().length() != 11)
				{
					Toast.makeText(getApplicationContext(), "用户电话号码位数不正确!", 2000).show();
					return;
				}
			}
			...
			new Thread() {
				public void run() {
					RegisterThead(
							...
							etUserPwd2.getText().toString().trim(), 
	...
	private void RegisterThead(String userCode,String userName, String password,String phone) {
		try { 
	...
	final Runnable mUpdateResults = new Runnable() {
		public void run() {
			...
			{
				Toast.makeText(getApplicationContext(), "注册成功,请牢记帐号及密码！", 2).show();
				finish();
			}
			else
			{
				Toast.makeText(getApplicationContext(), "注册失败:" + ErrMsg, 10).show();
			}
