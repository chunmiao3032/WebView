...
//						CardShow_DtlInfoActivity.class);
//				intent.putExtra("card",(Serializable)card);
//				startActivity(intent);
				
				Intent intent = new Intent(CardShowActivity.this,WebViewActivity.class);
	...
	String[] menu1 = new String[] { "全部", "中信银行","交通银行","浦发银行","农业银行","广发银行",
			"光大银行","招商银行","平安银行","兴业银行","花旗银行" };
	final String[] menu2 = new String[] { 
			"全部", "20-25岁","26-35岁","36-45岁","46岁以上"
	}; 
	...
	private void initAdapter() {
		...
		items = WebServiceManager.getCards(
				Global.City, _Bank, _Level, _CardOrg,_Ages,
				"0",
				_IsNewUser,_IsFastGet,_IsHighQuotar);
...
//		tvRowCount.setText("为您找到相关结果约" + String.valueOf(count) + "张信用卡");
	...
	private void loadData(int lastIndex) {
		List<CardResult> items = WebServiceManager.getCards(Global.City, _Bank,
				_Level, _CardOrg,_Ages, String.valueOf(lastIndex),
				_IsNewUser,_IsFastGet,_IsHighQuotar);
